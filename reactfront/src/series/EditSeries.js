...
class EditSeries extends Component {
	...
	init = (seriesId) => {
		seriesById(seriesId).then((data) => {
			if (data.error) {
				...
				if (isAuthenticated().user._id === data.createdBy._id || isAuthenticated().user.role === "admin"){
					this.setState({
						id: data._id,
						name: data.name,
	...
	clickSubmit = (event) => {
		...
		update(seriesId, token, this.postData).then((data) => {
			if (data.error) {
	...
	createPost = (name, summary, image, nameError, summaryError, imageError, uploading, loading) => (
		<form className="p-4">
			<h1 className="text-2xl tracking-wide font-semibold text-white my-3">UPDATE SERIES</h1>
			...
			<div className="relative w-full mb-3">
				<label className="block uppercase tracking-wide text-cream text-xs font-bold mb-2">
					Series Summary:
				</label>
				<TextareaAutosize
