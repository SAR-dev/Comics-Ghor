...
import { clearLike } from './apiNotification';
...
export default class CommentNotification extends Component {
    ...
    remove = () => {
        ...
        clearLike(userId, token, postId, ownerId).then((data) => {
            if (data.error) {
                ...
                {!this.state.loading && (
                    <>
                        ...
                        <Link to={`/post/${post._id}`} className="block p-2">
                            <span>{user.name}</span> Clapped your post <span>{post.title.length > 50 ? `${post.title.slice(0,50)}...` : post.title}</span>
                        </Link>
                        </div>
