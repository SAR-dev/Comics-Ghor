...
class NewArtworks extends Component {
	constructor() {
		super();
	...
	imageArray = () => {
		return this.state.image.map((image, i) => {
			return (
				<div className="w-1/2 mb-2 hover:opacity-50 cursor-pointer px-1" key={i}>
					<img
						className="w-full h-48 rounded object-contain bg-gray-400 px-2"
	...
	clickSubmit = (event) => {
		...
		if (!this.state.titleError && !this.state.bodyError && this.state.seriesOf.length > 1 && this.state.catOf.length > 1) {
		this.setState({ loading: true });

		const userId = isAuthenticated().user._id;
		const token = isAuthenticated().token;
		this.postData.set('thumbnail', this.state.image[0]);
		this.postData.set('typeOf', 'artworks');
	...
	createPost = (series, cat, title, body, image, titleError, bodyError, imageError, uploading, loading) => (
		<form className="p-4">
			<h1 className="text-2xl tracking-wide font-semibold text-white my-3">Upload Artworks</h1>
			<div className="relative w-full mb-3">
				<input
					...
					placeholder="Give your artworks a title"
            ...
            <div className="relative w-full mb-3">
				...
				<div className="border border-dashed border-gray-500 relative">
					...
					<div className="text-center p-10 absolute top-0 right-0 left-0 m-auto">
						...
						<h4 className="text-gray-600 text-xl">Click and select artworks to upload</h4>
					</div>
				</div>
            ...
            <div className="relative w-full mb-3">
                ...
                <TextareaAutosize
					...
					value={body}
					type="text"
					className="h-32 resize-y px-3 py-3 placeholder-gray-500 text-gray-800 bg-gray-200 rounded text-sm focus:outline-none focus:shadow-outline w-full"
					onChange={this.handleTextBody("body")}
					minRows="5"
					style={{ transition: 'all 0.15s ease 0s' }}
				/>
                </div>
	...
	render() {
		...
		return (
			<>
			<SeoHelmet title="Upload Artworks" />
			<PrimaryLayout>
				<div
					className={
