...
import { imageupload, update, catById } from './apiCat';
...
class EditSeries extends Component {
	...
	clickSubmit = (event) => {
		...
		update(catId, token, this.postData).then((data) => {
			if (data.error) {
	...
	createPost = (name, summary, image, nameError, summaryError, imageError, uploading, loading) => (
		<form className="p-4">
			<h1 className="text-2xl tracking-wide font-semibold text-gray-700 my-3">UPDATE SERIES</h1>
